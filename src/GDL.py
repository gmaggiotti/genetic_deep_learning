...
init_pob = [NN(X, y,epochs) for i in range(population_size)]
...
for x in range(1,generations):
    ...
    for i in range(population_size):
        candidate1 = gen[x-1][np.random.randint(best_n_children)][1].get_weight()
        candidate2 = gen[x-1][np.random.randint(best_n_children)][1].get_weight()
