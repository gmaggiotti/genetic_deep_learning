...
def run_GDNN_model(nn_class, epochs, population_size, generations, dataset):
    ...
    init_pob = [nn_class(dataset, epochs) for i in range(population_size)]
    init_pob = sort_by_fittest([(nn.calc_accuracy(), nn) for nn in init_pob], Type.accuracy)
    acc, acc_std = init_pob[0][1].calc_accuracy()
    ...
    for x in range(1, generations):
        ...
        for i in range(population_size):
            ...
            aux = nn_class(dataset, epochs, w_child)
            population += [tuple((aux.calc_accuracy(), aux))]
        gen[x] = sort_by_fittest(population, Type.accuracy)
        net = gen[x][0][1]
        acc, acc_std = net.calc_accuracy()
